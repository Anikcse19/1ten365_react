...
const Complain = (props) => {
  ...
  const fetchLevel1Data = () => {
    if (upLevel1?.id) {
      axios.get(`${base_url}/admins/${upLevel1?.input_id}`).then((res) => {
        if (res?.data?.msg == "success") {
  ...
  const fetchLevel2Data = () => {
    if (upLevel2?.id) {
      axios.get(`${base_url}/admins/${upLevel2?.input_id}`).then((res) => {
        if (res?.data?.msg == "success") {
